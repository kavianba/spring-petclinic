...
```
tanzu apps workload create gevans-petclinic -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/gevans-petclinic-source \
  --type web
```
...
```
tanzu apps workload get gevans-petclinic
```
